...
class CustomPPOTrainer_SG_5S(PPOTrainer, Trainer):
    r"""
    Inherits PPOTrainer.
    """
    ...
    def __init__(
        ...
        self.batch_size_1time = self.config.batch_size
        self.batch_size_2times = 2 * self.config.batch_size
    ...
    def ppo_train(self, resume_from_checkpoint: Optional[str] = None) -> None:
        ...
        for step in tqdm(range(max_steps), disable=not self.is_local_process_zero()):
            ...
            if step % SELECTOR_STEP != 0:
                self.config.batch_size = self.batch_size_1time
            else:
                self.config.batch_size = self.batch_size_2times
            # print('self.config.batch_size', self.config.batch_size)

            for idx in range(0, self.batch_size_1time, self.config.mini_batch_size):  # 因为有selector和generator两个模块的更新，因此bs扩大一倍，所以这里需要 // 2
                ...
                for sub_idx in range(self.config.mini_batch_size):
                    # print(len(mini_batch["input_ids"]), sub_idx)
