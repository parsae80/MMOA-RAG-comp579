...
generator_model_path = '/root/paddlejob/workspace/env_run/rag/LLaMA-Factory/models/llama3-8b/{}/qr_selector_and_generator/ppo_2_04epochs'.format(dataset_name)
...
for i in range(len(mini_batch_subquestions)):
    subq_list = mini_batch_subquestions[i][:5]  # 改写后的query不超过5个
    ...
    # temp_doc_list = all_top_docs[:5]  # 原问题的前5 doc + subquestion的doc
...
for i in range(len(qr_answers)):
    subq_list = qr_answers[i][:5]  # 改写后的query不超过5个
    ...
    for subq in subq_list:
        temp_doc_list = temp_doc_list + qr_all_subquestions_docs_dict[subq][: num_docs_per_subq]
    shuffled_all_top_docs.append(temp_doc_list)
...
for i in tqdm(range(0, len(selector_messages_list), batch_size)):
    ...
    answers_batch = batch_get_response(model, tokenizer, messages_batch, logits_processor)
    # for answer in answers_batch:
...
# 保存答案为jsonl
with open(save_results_path+'/sft_2.jsonl', 'w') as file: # 
