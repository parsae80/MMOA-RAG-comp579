...
class CustomPPOTrainer_SG_slowG(PPOTrainer, Trainer):
    r"""
    Inherits PPOTrainer.
    """
    ...
    def __init__(
        ...
        # 因为有selector和generator两个模块的更新，因此bs扩大一倍
        # self.config.batch_size = 2 * self.config.batch_size
        self.batch_size_both = 2 * self.config.batch_size
        self.batch_size_single = self.config.batch_size
    ...
    # def get_selector_duplicate_reward(self, input_string):
    #     # 如果回答为空 直接返回0.0
    #     if input_string == '':
    #         return 0.0
    #     # 将字符串分割成数字列表
    ...
    #             # print('input_string 1: ', input_string)
    #             return -0.5
    #     # 检查是否有重复数字
    #     if len(numbers) != len(set(numbers)):
    #         # print('input_string 2: ', input_string)
    #         return -0.5
    ...
    def get_answer_dict(self, answers_path):
        ...
        for ans_pair in answers_pair:
            ...
            question_golden_answer_dict[ans_pair['question'].replace(" ", "")] = ans_pair['answer']  # hotpotqa 2wikimultihotpotqa
    ...
    def get_selector_rewards(self, predict_answers, golden_answers, mini_batch_candidate_docs, mini_batch_input_questions, mini_batch_selector_answers_text, K_candidate):

        assert len(predict_answers) == len(golden_answers), "预测答案和标准答案的长度不相等"

        rewards = []
        for i in range(len(predict_answers)):
            score = self.get_selector_duplicate_reward(mini_batch_selector_answers_text[i], K_candidate)  # 0.0 or -0.5 (惩罚输出重复内容和非doc id的内容)
    ...
    def get_selector_repeat_punish(self, mini_batch_selector_answers_text, K_candidate):
        rewards = []
    ...
    def ppo_train(self, resume_from_checkpoint: Optional[str] = None) -> None:
        ...
        for step in tqdm(range(max_steps), disable=not self.is_local_process_zero()):
            ...
            if step < 3:
                print('step: {}'.format(step), 'self.kl_ctl.value: {}'.format(self.kl_ctl.value))
            ...
            # print('self.config.mini_batch_size', self.config.mini_batch_size)

            if step % SELECTOR_STEP == 0:
                self.config.batch_size = self.batch_size_both
            else:
                self.config.batch_size = self.batch_size_single

            for idx in range(0, self.batch_size_single, self.config.mini_batch_size):  # 因为有selector和generator两个模块的更新，因此bs扩大一倍，所以这里需要 // 2
                ...
                for answer in mini_batch_selector_answers_text:
                    # answer = answer.replace("*", "")
                    number_answer = self.extract_digits(answer, K_candidate)
                    
                    # if step == 0 and idx == 0:
                    #     print(number_answer, answer)
                ...
                mini_batch_rewards_selctor_aindoc = self.get_selector_rewards(predict_answers, golden_answers, mini_batch_input_candidate_docs, mini_batch_input_questions, mini_batch_selector_answers_text, K_candidate)
                mini_batch_repeat_punish = self.get_selector_repeat_punish(mini_batch_selector_answers_text, K_candidate)

                ## 对应前面selector和generator两次的rewards 即共享rewards
                if step % SELECTOR_STEP == 0:
