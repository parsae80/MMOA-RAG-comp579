...
class CustomPPOTrainer(PPOTrainer, Trainer):
    ...
    def get_answer_dict(self, answers_path):
        ...
        for ans_pair in answers_pair:
            question_golden_answer_dict[ans_pair['question'].replace(" ", "")] = ans_pair['golden_answer']
    ...
    def get_rewards(self, predict_answers, golden_answers, reward_metric_name='em'):  # torch.rand((self.config.mini_batch_size, 1))
    ...
    def ppo_train(self, resume_from_checkpoint: Optional[str] = None) -> None:
        ...
        answers_path = '/root/paddlejob/workspace/env_run/rag/data/naive_rag/train_to_pair_6epochs_4096.jsonl'
        ...
        for step in tqdm(range(max_steps), disable=not self.is_local_process_zero()):
            ...
            # print("self.config.batch_size {}, self.config.mini_batch_size: {}".format(self.config.batch_size, self.config.mini_batch_size))
            ...
            for idx in range(0, self.config.batch_size, self.config.mini_batch_size):
                ...
                # mini_batch_rewards = self.get_rewards() # self.get_rewards(mini_batch_queries, mini_batch_responses)
                queries.extend(mini_batch_queries)  # 需要从中decode后，提取出question的原始文本
                responses.extend(mini_batch_responses)  # 需要decode出answer的文本；后续与load的原始数据的golden answer计算一些metrics当作rewards。

                golden_answers = []
                for input_ids in mini_batch_queries:
                    question = self.extract_question(self.tokenizer.decode(input_ids, skip_special_tokens=True))

                    with open('./question_output.txt', 'a', encoding='utf-8') as file:
                        # file.write(self.tokenizer.decode(input_ids, skip_special_tokens=True) + '\n')
                        file.write(question + '\n')
                ...
                predict_answers = []
                for response_ids in mini_batch_responses:
                ...
                mini_batch_rewards = self.get_rewards(predict_answers, golden_answers, reward_metric_name='em')
                rewards.extend(mini_batch_rewards)

                # print(predict_answers)
                # print(golden_answers)
                # print(mini_batch_rewards)
                # print('\t')
