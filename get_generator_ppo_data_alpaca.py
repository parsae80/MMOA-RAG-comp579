...
top_k_docs_path = '/root/paddlejob/workspace/env_run/rag/data/train_top_k_docs_debug.jsonl'
...
for i in tqdm(range(len(top_k_list))):
    ...
    history = []
    # history.append(["Before giving you question and documents, I show you somes examples.\n\nQuestion: Who was the first person killed in a car accident\nAnswer: Bridget Driscoll\n\nQuestion: Are both The New Pornographers and Kings of Leon American rock bands?\nAnswer: no\n\nQuestion: What is the length of the track where the 2013 Liqui Moly Bathurst 12 Hour was staged?\nAnswer: 6.213 km long\n\nQuestion: Which was the first European country to abolish capital punishment?\nAnswer: Norway\n\nPlease answer the question briefly like above examples.", "Okay, please provide the question and the corresponding documents."])
    # history.append(["Question is: {}".format(str(question)), "Received Question."])
    # for doc_id in range(len(top_k_docs)):
    #     doc_content = top_k_docs[doc_id]['content']
    #     history.append(["Document {}: {}".format(str(doc_id), str(doc_content)), "Received Document {}.".format(str(doc_id))])
...
# 保存答案为jsonl
with open(save_results_path+'/train_ppo_generator_data_debug.json', 'w') as file:
