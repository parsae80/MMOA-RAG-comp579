...
qr_model_path = '/root/paddlejob/workspace/env_run/rag/LLaMA-Factory/models/llama3-8b/{}/qr_selector_and_generator/ppo_1'.format(dataset_name)
...
# loading query rewriting model
print('*'*20)
print('loading generator model')
model_qr = AutoModelForCausalLM.from_pretrained(
    qr_model_path,
    torch_dtype="auto",
    device_map="auto"
)
model_qr.eval()
model_qr = model_qr.to(torch.float16)
tokenizer_qr = AutoTokenizer.from_pretrained(qr_model_path)
tokenizer_qr.padding_side = "left"
end_time = time.time()
print('time consuming: {} seconds'.format(end_time - start_time))
...
for i in tqdm(range(0, len(qr_messages_list), batch_size)):
    ...
    answers_batch = batch_get_response(model_qr, tokenizer_qr, messages_batch)
...
for i in range(len(qr_answers)):
    ...
    num_docs_per_subq_dict = {'1': [10], '2': [5,5], '3': [4,3,3], '4': [3,3,2,2], '5': [2,2,2,2,2]}  # 10个候选文档
...
# 保存答案为jsonl
with open(save_results_path+'/RRR.jsonl', 'w') as file: # 
