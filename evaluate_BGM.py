...
generator_model_path = '/root/paddlejob/workspace/env_run/rag/LLaMA-Factory/models/llama3-8b/{}/qr_selector_and_generator/sft_1'.format(dataset_name)
...
selector_model_path = '/root/paddlejob/workspace/env_run/rag/LLaMA-Factory/models/llama3-8b/{}/qr_selector_and_generator/ppo_1'.format(dataset_name)
...
print('*'*20)
print('loading generator model')
model_selector = AutoModelForCausalLM.from_pretrained(
    selector_model_path,
    torch_dtype="auto",
    device_map="auto"
)
model_selector.eval()
model_selector = model_selector.to(torch.float16)
tokenizer_selector = AutoTokenizer.from_pretrained(selector_model_path)
tokenizer_selector.padding_side = "left"
end_time = time.time()
print('time consuming: {} seconds'.format(end_time - start_time))
...
allowed_token_ids = tokenizer_selector.convert_tokens_to_ids(allowed_tokens)
eos_token_id = tokenizer_selector.eos_token_id
...
for i in tqdm(range(0, len(selector_messages_list), batch_size)):
    ...
    for j in range(len(answers_batch)):
        ...
        number_answer = extract_digits(answer, K_candidate=len(shuffled_all_top_docs[i+j]))
        ...
        with open("./sft_1epoch_selector.txt", "a", encoding="utf-8") as file:
            file.write(answer + " || " + str(len(shuffled_all_top_docs[i+j])) + '\n')
...
# 保存答案为jsonl
with open(save_results_path+'/BGM.jsonl', 'w') as file: # 
